...
try{
	...
	if( trim($_POST['text']) == "" )
	{
	 	$stmt = $pdo->query('SELECT * from photos where del_status = 0  ORDER BY RAND() LIMIT '.$lim_start.','.$f_ar['current'].' ');
	...
	{	
		...
		$q = 'SELECT * from photos  left join img_meta on img_meta.pic_id = photos.id_from_src  where photos.del_status = 0 and (';
		// $stmt = $pdo->query(
